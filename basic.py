...
class Error:
  ...
  def as_string(self):
    ...
    result += f'File {self.pos_start.funcion}, line {self.pos_start.linea + 1}'
    result += '\n\n' + string_with_arrows(self.pos_start.funcion_texto, self.pos_start, self.pos_end)
    return result
...
class Lexer:
  def __init__(self, funcion, text):
    self.funcion = funcion
    self.text = text
    self.pos = Posicion(-1, 0, -1, funcion, text)
  ...
  def make_tokens(self):
    ...
    while self.caracter_actual != None:
      if self.caracter_actual in ' \t':
        ...
        tokens.append(Token(TT_PARENIZQ, pos_start=self.pos))
        self.avanzar()
      elif self.caracter_actual == ')':
        tokens.append(Token(TT_PARENDER, pos_start=self.pos))
        self.avanzar()
      elif self.caracter_actual == '[':
...
class Parser:
  ...
  def statements(self):
    ...
    return res.success(ListNode(
      statements,
      pos_start,
      self.current_tok.pos_end.copiar()
    ))
  ...
  def statement(self):
    ...
    if self.current_tok.matches(TT_KEYWORD, 'RETORNA'):
      res.register_advancement()
      self.avanzar()
      ...
      if not expr:
        self.reverse(res.to_reverse_count)
      return res.success(ReturnNode(expr, pos_start, self.current_tok.pos_start.copiar()))
    
    if self.current_tok.matches(TT_KEYWORD, 'CONTINUA'):
      res.register_advancement()
      self.avanzar()
      return res.success(ContinueNode(pos_start, self.current_tok.pos_start.copiar()))
      
    if self.current_tok.matches(TT_KEYWORD, 'ROMPER'):
      res.register_advancement()
      self.avanzar()
      return res.success(BreakNode(pos_start, self.current_tok.pos_start.copiar()))
  ...
  def comp_expr(self):
    ...
    if self.current_tok.matches(TT_KEYWORD, 'NEL'):
  ...
  def call(self):
    ...
    if self.current_tok.type == TT_PARENIZQ:
      res.register_advancement()
      self.avanzar()
      arg_nodes = []

      if self.current_tok.type == TT_PARENDER:
        res.register_advancement()
        self.avanzar()
      else:
  ...
  def if_expr_b_or_c(self):
    ...
    if self.current_tok.matches(TT_KEYWORD, 'SINOSI'):
...
class Number(Value):
  ...
  def copiar(self):
    copiar = Number(self.value)
    copiar.set_pos(self.pos_start, self.pos_end)
...
class Interpreter:
  ...
  def visit_VarAccessNode(self, node, context):
    ...
    value = value.copiar().set_pos(node.pos_start, node.pos_end).set_context(context)
  ...
  def visit_UnaryOpNode(self, node, context):
    ...
    elif node.op_tok.matches(TT_KEYWORD, 'NEL'):
  ...
  def visit_CallNode(self, node, context):
    ...
    if res.should_return(): return res
    value_to_call = value_to_call.copiar().set_pos(node.pos_start, node.pos_end)
    ...
    if res.should_return(): return res
    return_value = return_value.copiar().set_pos(node.pos_start, node.pos_end).set_context(context)
    return res.success(return_value)
...
def run(funcion, text):
  # Generate tokens
  lexer = Lexer(funcion, text)
...
""" def generate_object_code(funcion, text, output_filename):
  ...
  object_code = []
  for token in tokens:
      object_code.append(f"{token.type}: {token.value}" if token.value else f"{token.type}")
